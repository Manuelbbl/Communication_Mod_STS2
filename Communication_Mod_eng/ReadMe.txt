Description:
Are you looking to train an AI, build a custom bot, or just log extensive data from your Slay the Spire 2 runs? The StS2 AI Communication Bridge is a powerful API mod designed specifically for developers, AI researchers, and data nerds.

This mod seamlessly hooks into the core events of Slay the Spire 2 and exports the complete live game state as clean, structured JSON data via local HTTP POST requests. It acts as a perfect foundation for Reinforcement Learning (RL) or automated gameplay.

Key Features:

Comprehensive Combat Tracking: Real-time updates on player HP, energy, hand cards, draw/discard/exhaust piles, relics, and potions.

Deep Enemy Data: Detailed information on all enemies, including current HP, active buffs/debuffs, and their exact intents (move type, damage, and hit counts).

Out-of-Combat State: Complete tracking of non-combat events! The mod reads and exports data from the Map generation, Campfires, Events, Treasure rooms, and Merchant inventories (Shops).

Loot & Rewards: Scans and exports the reward screen, detailing gold, potions, relics, and specific card choices.

UI Focus Tracking: Tracks which UI element (node, card, enemy) is currently focused, enabling precise bot navigation.

Optimized for AI: Data is structured specifically for machine learning models, creating dynamic card catalogs and preventing redundant data spam during enemy turns.

How it works:
The mod automatically sends JSON payloads to a local server (default: [http://127.0.0.1:5000/update_state](http://127.0.0.1:5000/update_state)). You can easily catch this data using a lightweight Python Flask server to feed it directly into your PyTorch/TensorFlow models or bot scripts.

Ready to let the machine conquer the Spire?
